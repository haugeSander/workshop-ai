...
/*
 * Kroppen til hver typeerklæring i en fil, med nøstingen intakt.
 *
 * Uten den leste feltsøket under `const koder: Begrunnelseskode[] = []` som et
 * felt som het `koder`, og krevde at noen leste `.koder` et sted. En lokal
 * variabel er ikke et felt, og en parameter er det heller ikke: da ble
 * BEGRUNNELSESKODER rødt av at regelen faktisk brukte kodeverket, som er det
 * motsatte av det sjekken er til for. Kodeverket lå i en tjeneste før og ble
 * derfor aldri skannet - heuristikken har aldri vært prøvd før nå.
 *
 * `[^\n{]*` foran klammen er der for `type Forslag = Skaaring & {`: en
 * skjæringstype er like mye en typeerklæring som en ren objekttype.
 */
function typekropper(kilde: string): string[] {
  const kropper: string[] = [];
  const start = /(?:^|\n)\s*(?:export\s+)?(?:type\s+\w+\s*=|interface\s+\w+)[^\n{]*\{/g;
