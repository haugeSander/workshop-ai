 ...
 */
function avvisning(utfall: Overgangsutfall<string>): { status?: number; kode?: string; melding?: string } {
  return utfall.lovlig ? {} : utfall;
}
...
for (const fra of SAMTYKKESTATUSER) {
  for (const til of SAMTYKKESTATUSER) {
    ...
    check(`${fra} → ${til} gir 409`, utfall.lovlig === false && utfall.status === 409, String(avvisning(utfall).status));
  }
}
...
check("ukjent status gir 400", !tull.lovlig && tull.status === 400, String(avvisning(tull).status));
check("ukjent status gir kode UKJENT_STATUS", !tull.lovlig && tull.kode === "UKJENT_STATUS");
check("ukjent status lister de gyldige", !tull.lovlig && tull.melding.includes("VENTER_PAA_SVAR"), avvisning(tull).melding);

// Rubbish already on disk: the request is fine, the row is not.
const raatten = validateSamtykkeovergang("SAMTYKKA", "TRUKKET");
check("ukjent lagret status gir 409", !raatten.lovlig && raatten.status === 409, String(avvisning(raatten).status));

// --- 3. utløp --------------------------------------------------------------

check("utløp i framtiden er ikke utløpt", isUtloept({ utloper: iTida(1) }, TEST_NOW) === false);
check("utløp i fortiden er utløpt", isUtloept({ utloper: iTida(-1) }, TEST_NOW) === true);
...
async function call(sti: string, valg: Kallvalg = {}) {
 ...
 */
function fiksToken(scope: string) {
  return {
...
function tokenForSti(sti: string) {
...
type Revisjonshendelse = {
  ...
  grunnlag?: { id?: string; status?: string; forsoekt?: string; kode?: string };
};

const revisjon: Revisjonshendelse[] = [];
const revisjonstjener = createServer((request, response) => {
  ...
  request.on("end", () => {
    revisjon.push(JSON.parse(kropp) as Revisjonshendelse);
...
const tjeneste = spawn(process.execPath, [path.join(repoRoot, "apps/fiks-simulator/src/server.ts")], {
  cwd: repoRoot,
  env: {
    ...process.env,
    PORT: String(port),
...
try {
  ...
  check(
    ...
    [...unike].every((kandidat) => etter.some((rad: any) => rad.samtykkeId === kandidat))
  );
  ...
  const onDiskAfter = JSON.parse(await readFile(samtykkeFil, "utf8"))
    .find((kandidat: any) => kandidat.samtykkeId === expiringId);
  ...
  // 6k. revisjonsloggen
  const hendelser = (handling: any) => revisjon.filter((h) => h.handling === handling);
